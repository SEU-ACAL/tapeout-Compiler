...
public:
  ...
  LogicalResult matchAndRewrite(linalg::Conv2DNhwcFhwcOp convOp,
    ...
    MemRefType biasType = MemRefType::get(outputShape[3], rewriter.getI32Type());
    if (accType == "f32")
    ...
    Value kernelDim =
        rewriter.create<arith::ConstantIndexOp>(loc, kernelShape[1]); // dim_h = dim_w
    Value inChannels =
        rewriter.create<arith::ConstantIndexOp>(loc, kernelShape[3]);

    // Conv kernel mapping (f,h,w,c) -> (h*w*c, f)
    Value tmp0 = rewriter.create<arith::MulIOp>(loc, loopIvs[1], kernelDim);  // h * kW
    tmp0 = rewriter.create<arith::MulIOp>(loc, tmp0, inChannels);             // * C
    Value tmp1 = rewriter.create<arith::MulIOp>(loc, loopIvs[2], inChannels); // w * C
    ...
    rewriter.create<memref::StoreOp>(loc, element, kernelMat, indices);  // Store the loaded data
